...
vec3 refine(vec3 a, vec3 b, float isosurface, float direction)
{
    ...
    {
        vec3 m = 0.5 * (a + b);
        float v = (isoValue(m) - isosurface) * direction;
        if (v >= 0.0)   b = m;
        else            a = m;
    }
    return b;
...
//    float t_exit = exit(vPosition.xyz, raydir);
    ...
    float alphaThreshold = 3.0;
    float alphaRatio = 0.2;
    ...
    for (float t = t_entry; t < 0.0; t += t_step, tc += tc_step)
    {
        ...
        vec3 colour = shade(position, view, normal) * texColor.rgb;

        float gradMag = length(nabla);
        // Transfer function for opacity (alpha)
        float grad_alpha = clamp(gradMag * 0.1, 0.0, 1.0); // could also use intensity
        float tex_alpha = clamp( (intensity - uIsosurface) * 5.0, 0.0, 1.0 );

        // alphaRatio = grad_alpha * tex_alpha;
//            sum = vec4(normalize(nabla), 1.0);
        sum = (1.0 - alphaRatio) * vec4(colour, texColor.r) + alphaRatio * sum;
